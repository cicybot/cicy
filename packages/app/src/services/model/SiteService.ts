...
export class SiteService {
    ...
    static async initDb() {
        if (!localStorage.getItem('site_db_inited')) {
            ...
            localStorage.setItem('site_db_inited', 'true');
        }
    }
    ...
    async saveSiteInfo(site: SiteInfo) {
        ...
        if (res) {
            ...
            await new DatabaseService().run(
                `INSERT INTO site (site_id, url, no_webview, title, icon, updated_at)
                 VALUES (?, ?, ?, ?, ?, ?)`,
                [
                    site.site_id,
                    site.url,
    ...
    async getAccounts(): Promise<SiteAccountInfo[]> {
        const rows = await SiteAccount.getRows(this.siteId);
        return rows || [];
    }

    async getAccount(): Promise<SiteAccountInfo | null> {
        return new SiteAccount(this.siteId, this.accountIndex).get();
    }
