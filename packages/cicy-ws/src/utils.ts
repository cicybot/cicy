...
export function getLocalIPAddress() {
    const res = getLocalIPAddressList();
    return res.length > 0 ? res[0] : null;
}

export function getLocalIPAddressList() {
        ...
        if (addresses) {
            for (const addressInfo of addresses) {
                ...
                if (addressInfo.family === 'IPv4' && !addressInfo.internal) {
                    res.push({ adr: addressInfo.address, interfaceName: interfaceName });
                }
            }
        }
    }
...
export async function isPortOnline(port: number): Promise<boolean> {
    ...
    try {
        if (platform === 'win32') {
            ...
            try {
                const { stdout } = await execPromise(`lsof -i tcp:${port} -s TCP:LISTEN -t`);
                return stdout.trim() !== '';
            } catch {
